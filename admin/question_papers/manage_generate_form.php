...
$true_false = $conn->query("SELECT * FROM question_list where question_paper_id = '{$id}' and `type` = 4 ")->num_rows;
?>
<div class="container-fluid">
	<form action="./?page=question_papers/generated_question_paper" method="POST" id="type-form">
		<input type="hidden" name ="id" value="<?php echo isset($id) ? $id : '' ?>">
		    <div class="form-group">
                <label for="single" class="control-label">No. of Single Answer Questions to Generate(Choose any One )</label>
            ...
            <div class="form-group">
                <label for="multiple" class="control-label">No. of Multiple Answers Questions to Generate (Multiple correct) </label>
            ...
            <div class="form-group">
                <label for="text" class="control-label">No. of Text Answer Questions to Generate(Answer the following question ) </label>
            ...
            <div class="form-group">
                <label for="text" class="control-label">No. of True and False </label>
                <input type="number" name="text" id="text" min= "0" max = '<?= $text_max ?>' class="form-control form-control-sm rounded-0"  required/>
                <small class="text-primary"><em><?= format_num($true_false) ?> Available Questions</em></small>
            </div>
        </div>
	</form>
</div>
